...
const Womens = ({ isOpen, remove, node }) => {
  const { allShopifyProduct } = useStaticQuery(graphql`
    query popularWomensProducts {
      allShopifyProduct {
        edges {
          node {
            tags
  ...
  const sorted = allShopifyProduct.edges
    .filter(edge => edge.node.tags.includes("womens"))
    .sort(
      (edgeA, edgeB) =>
      ...
      {isOpen && (
        ...
        >
          ...
          <nav>
            <Link
              className="nav-item"
              to="product-list/gold"
              state={{ tag: "chrono", gender: "womens" }}
            >
              Gold Watches
            </Link>
            <Link
              className="nav-item"
              to="product-list/silver"
              state={{ tag: "chrono", gender: "womens" }}
            >
              Silver Watches
            </Link>
            <Link
              ...
              state={{ tag: "vintage", gender: "womens" }}
            >
              Vintage Watches
            </Link>
            <Link
